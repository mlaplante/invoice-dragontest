...
const Templates = () => {
  ...
  useEffect(() => {
    ...
    if (editingInvoiceStr) {
      ...
      setInvoiceId(editingInvoice.id)
      setFormData(editingInvoice.formData)
      setRows(editingInvoice.rows)
      setTemplate(editingInvoice.template)
      setTemplateSelected(true)
      setCurrencySymbol(editingInvoice.currencySymbol || '$')
      localStorage.removeItem('invoiceDragon_editing')
      return
    }
    ...
    if (!editingInvoiceStr) {
      ...
      if (allInvoices.length > 0) {
        ...
        const nextNumber = generateNextInvoiceNumber(lastInvoice.InvoiceNo)
        setFormData((prev) => ({
          ...prev,
  ...
  // Auto-save invoice draft when data changes
  useEffect(() => {
    if (templateSelected) {
      const id = invoiceId || `inv_${Date.now()}`
      if (!invoiceId) setInvoiceId(id)
  ...
  }, [formData, rows, template, currencySymbol, total, templateSelected, invoiceId])
  ...
  }, [formData, rows, showToast, handleDownloadCompleted])
                      ...
                      return isFormValid ? (
                        ...
                        >
                          {({ blob, url, loading, error }) => (
                            <button
                              className={`${styles.actionBtn || styles.action__btn} ${loading ? styles.loading : ''}`}
                              disabled={loading}
